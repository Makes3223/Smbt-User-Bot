...
async def help_cmd(_, message: Message):
    if len(message.command) == 1:
        msg_edited = False
        text = (
            "For more help on how to use a command, type <code>{prefixes}help [module]</code>\n\n"
            "Available Modules:\n"
        )
        ...
        for module_name, module_commands in modules_help.items():
            text += "• {}: {}\n".format(
                ...
                " ".join(
                    [
                        f"<code>{prefixes + cmd_name.split()[0]}</code>"
            ...
            for command in commands.keys():
                if command.split()[0] == command_name:
                    ...
                    return await message.edit(
                        f"<b>Help for command <code>{prefixes}{command_name}</code>\n"
                        f"Module: {name} (<code>{prefixes}help {name}</code>)</b>\n\n"
                        f"<code>{prefixes}{cmd[0]}</code>"
