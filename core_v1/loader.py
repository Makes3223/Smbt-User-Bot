...
@app.on_message(filters.command(["modhash", "mh"], prefixes) & filters.me)
...
@app.on_message(filters.command(["loadmod", "lm"], prefixes) & filters.me)
async def loadmod(_, message: Message):
    ...
    if len(message.command) > 1:
        ...
        if url.startswith(
            "https://raw.githubusercontent.com/Makes3223/Smbt-User-Bot_moduls/main/"
        ):
            module_name = url.split("/")[-1].split(".")[0]
        elif "/" not in url and "." not in url:
            module_name = url.lower()
            url = f"https://raw.githubusercontent.com/Makes3223/Smbt-User-Bot_moduls/main/{module_name}.py"
        else:
            modules_hashes = requests.get(
                "https://raw.githubusercontent.com/Makes3223/Smbt-User-Bot_moduls/main/modules_hashes.txt"
            ).text
            resp = requests.get(url)
            ...
            if hashlib.sha256(resp.content).hexdigest() not in modules_hashes:
                return await message.edit(
                    "<b>Only <a href=https://github.com/Makes3223/Smbt-User-Bot_moduls/tree/main/modules_hashes.txt>"
                    "verified</a> modules or from the official "
                    "<a href=https://github.com/Makes3223/Smbt-User-Bot_moduls>"
        ...
        if not os.path.exists(f"{BASE_PATH}/scripts/"):
            os.mkdir(f"{BASE_PATH}/scripts/")

        with open(f"./scripts/{module_name}.py", "wb") as f:
        ...
        if hashlib.sha256(content).hexdigest() not in modules_hashes:
            ...
            os.rename(file_name, f"./scripts/{module_name}.py")

    await message.edit(f"<b>The module <code>{module_name}</code> is loaded!</b>")


@app.on_message(filters.command(["unloadmod", "ulm"], prefixes) & filters.me)
async def unload_mods(_, message: Message):
    ...
    if os.path.exists(f"{BASE_PATH}/scripts/{module_name}.py"):
        os.remove(f"{BASE_PATH}/scripts/{module_name}.py")
        await message.edit(f"<b>The module <code>{module_name}</code> removed!</b>")
    elif os.path.exists(f"{BASE_PATH}/scripts/{module_name}.py"):
        await message.edit(
...
@app.on_message(filters.command(["loadallmods"], prefixes) & filters.me)
async def load_all_mods(_, message: Message):
    ...
    modules_list = requests.get(
        "https://api.github.com/repos/Makes3223/Smbt-User-Bot_moduls/contents/"
    ).json()
    ...
    for module_info in modules_list:
        if not module_info["name"].endswith(".py"):
            continue
        if os.path.exists(f'{BASE_PATH}/scripts/{module_info["name"]}'):
            continue
    ...
    for name, url in new_modules.items():
        with open(f"./scripts/{name}.py", "wb") as f:
...
@app.on_message(filters.command(["updateallmods"], prefixes) & filters.me)
async def updateallmods(_, message: Message):
    ...
    modules_installed = list(os.walk("scripts/"))[0][2]
    ...
    for module_name in modules_installed:
        ...
        resp = requests.get(
            f"https://raw.githubusercontent.com/Makes3223/Smbt-User-Bot_moduls/main/{module_name}"
        )
        if not resp.ok:
            modules_installed.remove(module_name)
            continue

        with open(f"./scripts/{module_name}", "wb") as f:
            f.write(resp.content)
